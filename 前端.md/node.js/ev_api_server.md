...
1. 在 `app.js` 中，全局中间件放在中间件最前面，为 res 对象挂载一个 `res.cc()` 函数 ：
...
// 新版---------------------------------------------------------------------------------------------------
// 解析 token 的中间件，正确导入方式（新版）
const { expressjwt } = require('express-jwt');
// 使用 .unless({ path: [/^\/api\//] }) 指定只有以 login 开头的接口不需要进行 Token 的身份认证
app.use(expressjwt({ secret: config.jwtSecretKey, algorithms: ['HS256'] }).unless({ path: [/^\/login/] }))
```
...
// 如果找不到 id，把 req.user 换成 req.auth 试试
...
![image](assets/image-20250522164307-23bzvgz.png)

#### 4.1.2 新增两条初始数据

![image](assets/image-20250522164318-9via851.png)
...
![image](assets/image-20250522194402-tzi7n1i.png)
...
> 当前项目，推荐使用 multer 来解析 `multipart/form-data` 格式的表单数据。[https://www.npmjs.com/package/multer](https://www.npmjs.com/package/multer)
