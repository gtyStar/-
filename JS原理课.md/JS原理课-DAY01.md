...
4. 剩余参数-[文档链接](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Functions#剩余参数)
...
>
> [传送门:继承（计算机科学）](https://zh.wikipedia.org/wiki/继承_(计算机科学))
>
...
1. 如何定义及使用[类](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Classes#类声明):
2. 如何定义实例属性/方法:
...
2. 内部直接写实例属性,可以设置默认值,
3. 实例方法的添加方式为`方法名(){}`
4. 构造函数通过`constructor`进行添加
5. 通过`new 类名()`创建实例,会调用构造函数`constructor`
...
2. 可以通过`JSON.stringify`将对象转为`JSON`
